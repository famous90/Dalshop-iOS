...
/*!
 ...
 @constant  KOErrorDeactivatedSession 세션이 만료된(access_token, refresh_token 이 모두 만료된 경우) 상태
 @constant  KOErrorAlreadyLoginedUser 로그인 된 상태 (access_token, refresh_token 이 존재 하는 경우)에서 다시 로그인 하려고 할때 발생하는 오류
 ...
 @constant  KOErrorBadParameter 패러미터 이상
 @constant  KOServerErrorUnknown 일반적인 서버 오류 응답 값. message를 확인해야 함.
 @constant  KOServerErrorBadParameter 패러미터 이상
 @constant  KOServerErrorApiLimitExceed API 호출수 제한 초과
 @constant  KOServerErrorNotSignedUpUser 미가입(가가입) 사용자
 @constant  KOServerErrorAlreadySignedUpUser 이미 가입된 사용자. 가입된 사용자에 대해 다시 가입요청한 경우.
 ...
 @constant  KOServerErrorInvalidAccessToken access_token 이 비정상적이거나 만료된 경우
 @constant  KOServerErrorInsufficientScope scope 을 벗어난 api 를 호출했을때
 @constant  KOServerErrorNotTalkUser 카카오톡 유저가 아닐때.
 @constant  KOServerErrorNotStoryUser 카카오스토리 유저가 아닐때.
 @constant  KOServerErrorStoryImageUploadSizeExceed 카카오스토리 이미지 업로드 사이즈 제한 오버.
 @constant  KOServerErrorUnderMaintenance 서버 메인터넌스 중.
 */
typedef enum {
    KOErrorUnknown = 1,
    KOErrorCancelled = 2,
